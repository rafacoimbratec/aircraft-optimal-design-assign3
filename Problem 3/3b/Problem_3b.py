...
def create_problem(surface, flight_conditions, design_vars):
    ...
    # prob.model.add_constraint('aero_point_0.wing_perf.cl', upper=1.2)
...
def run_optimization_case(case_name, design_vars, surface, flight_conditions):
    ...
    try:
        ...
        span = 2.0 * np.max(mesh_final[:, :, 1]) if surface['symmetry'] else np.max(mesh_final[:, :, 1]) - np.min(mesh_final[:, :, 1])
